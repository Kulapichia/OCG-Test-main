...
describe('apiUrl', () => {
  test('默认使用公共代理', () => {
    ...
    expect(window.getApiUrl('openai', '/models')).toBe('https://openai.chiangma.com/v1/models');
    expect(window.getApiUrl('claude', '/messages')).toBe('https://claudet.chiangma.com/v1/messages');
    expect(window.getApiUrl('gemini', '/models')).toBe('https://geminit.chiangma.com/v1beta/models');
  });
  ...
  test('占位符随 API 类型变化', () => {
    ...
    expect(proxy.placeholder).toBe('https://openai.chiangma.com/v1');
    expect(gemOnly.classList.contains('hidden')).toBe(true);
    document.getElementById('apiType').value = 'claude';
    window.updateProxyPlaceholder();
    expect(proxy.placeholder).toBe('https://claudet.chiangma.com/v1');
    expect(gemOnly.classList.contains('hidden')).toBe(true);
    document.getElementById('apiType').value = 'gemini';
    window.updateProxyPlaceholder();
    expect(proxy.placeholder).toBe('https://geminit.chiangma.com/v1beta');
